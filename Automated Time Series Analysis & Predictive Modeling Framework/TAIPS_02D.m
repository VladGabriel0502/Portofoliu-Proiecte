...
%% GWO configuration AR model
dim = 2; % 2 variabile - p si na, maximizam PQ bazat pe acestea doua
N_max = floor(N_antrenament_2/3) + 1;
lb = [ 0, 0 ]; % lower bound lb(1) lb for p and lb(2) for na
ub = [ bestPolyDeg(y_training_om), N_max]; % uper bound similar to lb 
SearchAgents_no = 30 ; % Standard
Max_iter = 200  ; % Big enough to find a good result
[Alpha_score, Alpha_pos] = GWO_pred_AR( SearchAgents_no, Max_iter, lb, ub, dim, y_training_om, y_training_op )
p_best = Alpha_pos(1);
na_best = Alpha_pos(2);
...
%% GWO configuration ARMA model
dim = 4; % 2 variabile - p si na, maximizam PQ bazat pe acestea doua     
N_max = floor(N_antrenament_2/3) + 1;
lb = [ 0 0 0 0 ]; % lower bound lb(1) lb for p and lb(2) for na 
ub = [ bestPolyDeg(y_training_om) N_max N_max N_max]; % uper bound similar to lb
SearchAgents_no = 30 ; % Standard
Max_iter = 100  ; % Big enough to find a good result
[Alpha_score, Alpha_pos] = GWO_pred_ARMA( SearchAgents_no, Max_iter, lb, ub, dim, y_training_om, y_training_op )
p_best = Alpha_pos(1)
na_best = Alpha_pos(2)
nc_best = Alpha_pos(3)
nalpha_best = Alpha_pos(4)
%% Grafice Strategie 3 -PARMA
% We display the coefficients of best polynomial and the Schuster
% Coefficients
% Outputs - trend
clc
...
v_om = v(1:N_Training);
nalpha = nalpha_best;
[ ~, e_AR, ~, theta_AR, yARMA, e_ARMA, lambda2_ARMA, theta_ARMA, na_top1, nc_top1 ] = stochastic(v_om, na_best, nc_best, nalpha);
AR_component = theta_ARMA(1:na_best + 1);
MA_component = theta_ARMA(na_best + 2:end);
% Calculating yARMA_pred
yARMA(N_Training + 1:N) = zeros(k,1);
for i = 1:k
    % Calculating the AR part
    for l = 1:na_top1
        if i <= l 
            yARMA(N_Training + i) = yARMA(N_Training + i) - AR_component(l + 1) * v_om(N_Training + i - l);
        else
            yARMA(N_Training + i) = yARMA(N_Training + i) - AR_component(l + 1) * yARMA(N_Training + i - l);
        end
    end
            
    % Calculating the MA part
    for l = 1:nc_top1
        yARMA(N_Training + i) = yARMA(N_Training + i) + MA_component(l + 1) * e_AR(N_Training + i - l);
    end
                         
    % Calculating the predicted noise
    if i == 1
        e_AR(N_Training + i) = v(N_Training + i - 1);
    else
        e_AR(N_Training + i) = yARMA(N_Training + i - 1);
    end
    for l = 1:nalpha
        if i <= l
            e_AR(N_Training + i) = e_AR(N_Training + i) + theta_AR(l + 1) * v_om(N_Training + i - l - 1);
        else
            e_AR(N_Training + i) = e_AR(N_Training + i) + theta_AR(l + 1) * yARMA(N_Training + i - l - 1);
        end
    end
end
...
% values as in the course
sigma2 = zeros(k,1); % 6 so that we can use recursion with sigma2(1) = sigma2(0) = 0
alpha_hat = InfiniteDivisionPoly(MA_component', AR_component', k);
for i = 2:k + 1
    sigma2(i) = sigma2(i - 1) + lambda2_ARMA * alpha_hat(i - 1)^2;
end
sigma = sqrt(sigma2);

y_estimated = y_Determinist;
y_estimated(N_Training + 1: N) = y_Determinist_est;
y_estimated = y_estimated + yARMA;
